                ...
                $(root).children().each(function (idxChild, tableChild) {
                    if (tableChild['type'] == 'tag') {
                        switch (tableChild['name']) {
                            ...
                            case 'tbody':
                            case 'tfoot':
                                $(tableChild).children('tr').each(function (idxRow, tr) {
                                    rows.push(tr);
                                });
                                break;
                            case 'tr':
    ...
    for (let key in definitions) {
        ...
        do {
            ...
            for (let i = content.length - 1; i >= 0; i--) {
                ...
                let contentToInsert = JSON.parse(JSON.stringify(dereferenced['content'].slice(1)));
                if (contentToInsert.length == 1) {
                    ...
                    for (let j = 0; j < contentToInsert.length; j++) {
                        ...
                        definition['depthMax'] =
                            Math.max(definition['depthMax'], content[i + j + 1]['depth']);
                    }
                    item[3] = null;
                }
...
function mergeAuxiliary(definition, dereferenced) {
    ...
    if (!('auxiliary' in definition)) {
        definition['auxiliary'] = JSON.parse(JSON.stringify(dereferenced['auxiliary']));
        return;
    }
